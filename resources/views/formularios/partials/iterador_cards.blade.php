        ...
        @foreach ($respuestas as $respuesta)
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card shadow-sm h-100 animacion_card border-0 overflow-hidden position-relative"
                    data-open-offcanvas="offcanvasAcciones" data-respuesta-id="{{ $respuesta->id }}"
                    data-form-id="{{ $formulario->id }}" data-grupo-id="{{ $respuesta->grupo_id ?? '' }}">
                    <div class="bg-info lateral_card"></div>

                    <div class="card-body contenido-ajustado"
                        style="max-height:400px; overflow-y:auto; padding: 0.85rem !important;">
                        ...
                        <div class="check-col_{{ $formulario->id }} d-none">
                            ...
                            <div class="form-check form-check-inline mb-0"
                                ...
                                <input class="form-check-input p-0 fila-checkbox_{{ $formulario->id }}" type="checkbox"
                                    value="{{ $respuesta->id }}" id="seleccion_{{ $respuesta->id }}"
                                    style="width:16px; height:16px;">
                                <label class="form-check-label mb-0"
                                    for="seleccion_{{ $respuesta->id }}">Seleccion</label>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            @if (
                                (isset($formulario->config['mostrar_usuario']) && $formulario->config['mostrar_fecha'] ?? false) ||
                                    (isset($formulario->config['mostrar_usuario']) && $formulario->config['mostrar_fecha'] ?? false))
