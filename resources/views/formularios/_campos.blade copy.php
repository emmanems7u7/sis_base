<div id="formulario-dinamico">
    <div class="row g-4">
        ...
        <div class="{{ $colClass }}">
            ...
            <div class="opciones-container" data-campo-id="{{ $campo->id }}">
                ...
                <div class="form-check">
                    <input type="checkbox" name="{{ $campo->nombre }}[]" value="{{ $opcion->catalogo_codigo }}"
                        ...
                        id="{{ $campo->nombre }}_{{ $opcion->catalogo_codigo }}" {{ in_array($opcion->catalogo_codigo,
            ...
            <button type="button" class="btn btn-sm btn-primary mt-2 btn-ver-mas-checkbox"
            ...
            <div class="radio-container" data-campo-id="{{ $campo->id }}">
                @foreach($campo->opciones_catalogo as $opcion)
                <div class="form-check">
                    <input type="radio" name="{{ $campo->nombre }}" id="{{ $campo->nombre }}"
                        value="{{ $opcion->catalogo_codigo }}" class="form-check-input campo-formulario"
                        id="{{ $campo->nombre }}_{{ $opcion->catalogo_codigo }}" {{ $valor==$opcion->catalogo_codigo ?
                    'checked' : '' }}>
                    <label class="form-check-label" for="{{ $campo->nombre }}_{{ $opcion->catalogo_codigo }}">
                        {{ $opcion->catalogo_descripcion }}
                    </label>
            ...
            <input type="file" name="{{ $campo->nombre }}" id="{{ $campo->nombre }}"
                data-preview="preview_{{ $campo->nombre }}" class="form-control" {{ $esRequerido && !$valor ? 'required'
                : '' }}>
            ...
            @if($valor)
            <div class="mb-2">
                <img src="{{ asset('archivos/formulario_'.$form.'/imagenes/'.$valor) }}" class="img-thumbnail"
            ...
            <input type="file" name="{{ $campo->nombre }}" id="{{ $campo->nombre }}" accept="image/*"
                data-preview="preview_{{ $campo->nombre }}" class="form-control" {{ $esRequerido && !$valor ? 'required'
                : '' }}>
            ...
            <input type="file" name="{{ $campo->nombre }}" id="{{ $campo->nombre }}" accept="video/*"
                data-preview="preview_{{ $campo->nombre }}" class="form-control" {{ $esRequerido && !$valor ? 'required'
                : '' }}>
                        ...
                        select.addEventListener('change', function () {
                            ...
                            fetch("{{ route('campos.obtenerData') }}", {
                                ...
                                .then(data => {
                                    ...
                                    // Buscar input que tenga el campo_referencia
