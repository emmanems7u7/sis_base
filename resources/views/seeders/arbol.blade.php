    ...
    @foreach($items as $item)
        <li class="mb-1">
            @if($item['tipo'] === 'carpeta')
                ...
                @endif
            @else
                <div class="seeder-item d-flex align-items-center justify-content-between" draggable="true"
                    data-ruta="{{ $item['ruta'] }}" ondragstart="dragSeeder(event)">
                    ...
                    <i class="fas fa-eye text-secondary ms-2" style="cursor: pointer;"
                        onclick="verSeeder('{{ $item['ruta'] }}', '{{ $item['nombre'] }}')" title="Ver seeder"></i>
                </div>
            @endif
        </li>
    @endforeach
