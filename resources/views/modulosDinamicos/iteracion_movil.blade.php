            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="headingMobile_{{ $formulario->id }}">
                    <button class="accordion-button {{ $formularioActivo != $formulario->id ? 'collapsed' : '' }}"
                        type="button" data-bs-toggle="collapse" data-bs-target="#collapseMobile_{{ $formulario->id }}"
                ...
                <div id="collapseMobile_{{ $formulario->id }}"
                    ...
                    <div class="accordion-body">
                        @include('modulosDinamicos.registros_movil', [
                            'item' => $item,
                            'modulo' => $modulo,
                        ])
                    </div>
        ...
        <div class="nav-wrapper position-relative end-0 ">
            <ul class="shadow-sm nav nav-pills nav-fill p-1" role="tablist">
                @foreach ($formulariosConRespuestas as $index => $item)
                    ...
                    <li class="nav-item">
                        <a class="nav-link mb-0 px-0 py-1 d-flex align-items-center justify-content-center  {{ $formularioActivo == $formulario->id ? 'active' : '' }}"
        ...
        <div class="mt-3">
            @foreach ($formulariosConRespuestas as $index => $item)
                ...
                <div id="formularioMobile_{{ $formulario->id }}"
                    class="formulario-tab-content  {{ $formularioActivo == $formulario->id ? 'fade show' : 'd-none' }}">
                    @include('modulosDinamicos.registros_movil', ['item' => $item, 'modulo' => $modulo])
                </div>
            @endforeach
        </div>
    ...
    <script>
        ...
        selectMobile.addEventListener('change', function() {
            ...
            const forms = document.querySelectorAll(
                '#formulariosSelectorMobile_{{ $modulo->id }} .formulario-tab-content');
